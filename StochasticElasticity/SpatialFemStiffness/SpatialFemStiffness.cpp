...
#include "SpatialFemStiffness.h"

REGISTER_OP("SpatialFemStiffness")
...
REGISTER_OP("SpatialFemStiffnessGrad")
...
class SpatialFemStiffnessOp : public OpKernel {
private:
  
public:
  explicit SpatialFemStiffnessOp(OpKernelConstruction* context) : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {    
    ...
    // TODO:
    DCHECK_EQ(hmat_shape.dim_size(0),*m_tensor**n_tensor);
    Forward fwd(hmat_tensor, *m_tensor, *n_tensor, *h_tensor);
    fwd.fill(context);

  }
};
REGISTER_KERNEL_BUILDER(Name("SpatialFemStiffness").Device(DEVICE_CPU), SpatialFemStiffnessOp);



class SpatialFemStiffnessGradOp : public OpKernel {
private:
  
public:
  explicit SpatialFemStiffnessGradOp(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("SpatialFemStiffnessGrad").Device(DEVICE_CPU), SpatialFemStiffnessGradOp);
...
#ifdef USE_GPU
class SpatialFemStiffnessOpGPU : public OpKernel {
private:
  
public:
  explicit SpatialFemStiffnessOpGPU(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("SpatialFemStiffness").Device(DEVICE_GPU), SpatialFemStiffnessOpGPU);

class SpatialFemStiffnessGradOpGPU : public OpKernel {
private:
  
public:
  explicit SpatialFemStiffnessGradOpGPU(OpKernelConstruction* context) : OpKernel(context) {
...
REGISTER_KERNEL_BUILDER(Name("SpatialFemStiffnessGrad").Device(DEVICE_GPU), SpatialFemStiffnessGradOpGPU);
